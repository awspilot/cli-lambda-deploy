...
Lambda.prototype.start = function( program ) {
	...
	setTimeout(function() {
		fs.watch(program, function(event, filename) {
			console.log("---| CONFIG ", ( process.env.AWSPILOT_DOCKER_WORKDIR ? 'DOCKER' : 'LOCAL' ), new Date().toISOString() )
			try {
				l.deploy( program )
			} catch (e) {}
		});
		...
		fs.watch( config.PATH, function(event, filename) {
			console.log("---| CODE ", ( process.env.AWSPILOT_DOCKER_WORKDIR ? 'DOCKER' : 'LOCAL' ), new Date().toISOString(), event, filename )
			try {
				l.deploy( program )
			} catch (e) {}
		});
